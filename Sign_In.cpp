...
void Log_in::connectDatabase()
{
    ...
    if (db.open()) {
        // TEMPORARY INSERT USER
        QSqlQuery insert;
        insert.prepare("INSERT INTO users (userName, password, email, phoneNumber, orderNumber) "
                       "VALUES ('Ellie', '1137', 'admin@example.com', '1234567890', 0)");
        if (!insert.exec()) {
            qDebug() << "Insert failed:" << insert.lastError().text();
        } else {
            qDebug() << "Test user inserted.";
        }
    }
}
